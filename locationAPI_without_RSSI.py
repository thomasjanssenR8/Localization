...
def get_data():
    ...
    print(bssids)
    data = [bssids, start_row, end_row, amount_of_bssids]
    return data
...
def write_combinations_to_file():
    ...
    for i in range(0, len(bssid_combs)):
        ...
        sheet.cell(row=row_index + i, column=3).value = bssid_combs[i][1]   # Write BSSID 2 in column D
...
def perform_request_of_combinations():
    ...
    for j in range(0, len(bssid_combs)):
        ...
        request.addAccessPoint(bssid_combs[j][0], "unknown")         # Add first AP (without RSSI)
        request.addAccessPoint(bssid_combs[j][1], "unknown")         # Add second AP (without RSSI)
        ...
        if response.status != 'Ok':
            ...
            sheet.cell(row=row_index+j, column=4).value = response.lat      # Write mean latitude of 2 BSSIDs in col D
            sheet.cell(row=row_index+j, column=5).value = response.lon      # Write mean longitude of 2 BSSIDs in col E
            sheet.cell(row=row_index+j, column=6).value = response.data['accuracy']  # Write accuracy (in m) in col F
...
def calc_error():
    ...
    for m in range(0, len(bssid_combs)):
        if mean_latitudes[m] and mean_longitudes[m]:  # if there is a mean, calculate the error and write to column G
            mean_coordinate = (mean_latitudes[m], mean_longitudes[m])
            distance = haversine(gps_coordinate, mean_coordinate)
            sheet.cell(row=row_index+m, column=7).value = distance

    # Write mean error of all pairs of BSSIDs above the table in column D
    sum = 0
    amount = 0
    for m in range(0, len(bssid_combs)):
        if sheet.cell(row=row_index+m, column=7).value:
            sum += sheet.cell(row=row_index+m, column=7).value
            amount += 1
    ...
    amount = 0
    for m in range(0, len(bssid_combs)):
        if sheet.cell(row=row_index+m, column=6).value:
            sum += sheet.cell(row=row_index+m, column=6).value
            amount += 1
...
def calc_chance():
    ...
    sheet.cell(row=end_row+7, column=4).value = chance_both_not_found       # write chance of no match to Excel
...
def show_map():
    ...
    gmap.draw("maps\\maps_locationAPI_without_RSSI\\BAP" + str(location) + ".html")            # Save the HTML file


#  Main program
#  --------------------------------------------------------------------------------------------------------------------
file = 'data\\data_locationAPI_without_RSSI.xlsx'               # Load Excel sheet of a location (e.g. BAP1)
book = openpyxl.load_workbook(filename=file)

for location in range(20, 21):   # (1, 37)                       # Load a template sheet for all 36 locations
    sheet = book.get_sheet_by_name('BAP' + str(location))

    [bssids, start_row, end_row, amount_of_bssids] = get_data()  # Retrieve collected data
