...
book_read = openpyxl.load_workbook(filename='data\\data_wigle.xlsx')  # Load Excel sheet with all location errors # @TODO change for LocationAPI
...
for location in range(1, 37):                                   # Load a template sheet for all 36 locations
    ...
    for error in errors:
        if error > 0.5:  # @TODO change value!
            amount__of_corrupt_errors += 1
    chance = amount__of_corrupt_errors / len(errors) * 100
    print('The chance of a corrupt bssid is %.2f' % chance, '%')
    sheet_write.cell(row=location+1, column=2).value = chance  # @TODO change column for LocationAPI
