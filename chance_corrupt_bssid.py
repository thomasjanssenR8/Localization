...
def get_data():
    ...
    for i in range(begin_combs, end_combs+1):
        if sheet.cell(row=i, column=7).value:
            errors.append(sheet.cell(row=i, column=7).value)       # Load distance errors (without None)
...
for location in range(1, 37):                                       # Load a template sheet for all 36 locations
    ...
    for error in errors:
        if error > 0.150:                                           # Threshold = 500 m for WiGLE, 150 m for LocationAPI
    ...
    sheet_write.cell(row=location+1, column=3).value = chance
